...
class ResetProgressCommand : SubCommand(), CustomKoinComponent {
    ...
    override fun onCommandByConsole(sender: CommandSender, alias: String, strings: Array<String>) {
        if(strings.size < 2) {
            sender.sendMessage("${ChatColor.RED}Please type a name after ${ChatColor.GOLD}reset${ChatColor.RED}. Command usage: /$alias $name <player>")
            return
        }

        // if player doesn't exist, warn and return
        val player = Bukkit.getPlayerExact(strings[1]) ?: run {
            sender.sendMessage("${ChatColor.RED}Player ${ChatColor.BLUE}${strings[1]}${ChatColor.RED} was not found, please check if you spelled his name correctly.")
            return
        }
